...
function ChatSection({ profile, groupData }) {
    ...
    const fetchNewMessage = async () => {
        try {
            ...
            const response = await fetch('http://50.112.48.169:3004/message/new', {
                method: 'POST',
                headers: {
    ...
    const fetchGetAllMessage = async () => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/message/team/${groupData[0].team.id}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${getToken()}`,
    ...
    useEffect(() => {
        if (groupData[0]) {
            // Khởi tạo socket
            socket.current = io('http://50.112.48.169:3004/message', {
                query: {
                    teamId: groupData[0]?.team?.id,
                    userId: profile.id,
                },
