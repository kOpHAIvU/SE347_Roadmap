...
function HeaderLogged({ collapsed, setCollapsed }) {
    ...
    const fetchOwnRoadmapData = async () => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/roadmap/owner?page=1&limit=12`, {
                method: 'GET',
                headers: {
    ...
    const handleCreate = async () => {
        if (name && description && image && userId) {
            ...
            try {
                const response = await fetch('http://50.112.48.169:3004/roadmap/new_roadmap', {
                    method: 'POST',
                    headers: {
