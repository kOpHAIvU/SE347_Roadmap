...
function CreateTimeline({ children, title, setTitle, content, setContent, handleOutsideClick, setShowDialog }) {
    ...
    const fetchRoadmapData = async (roadmapData) => {
        try {
            ...
            const response = await fetch(`http://50.112.48.169:3004/roadmap/item/${roadmapData.id}`, {
                method: 'PATCH',
                headers: {
    ...
    const fetchCloneRoadmap = async () => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/timeline/clone/${children.id}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${getToken()}`,
                    'Content-Type': 'application/json'
    ...
    const fetchUpdateTimelineTitleContent = async (timelineId, title, content) => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/timeline/item/${timelineId}`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${getToken()}`,
                    'Content-Type': 'application/json'
    ...
    const fetchNewTeam = async (name) => {
        try {
            const response = await fetch('http://50.112.48.169:3004/team/new', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${getToken()}`,
                    'Content-Type': 'application/json'
    ...
    const fetchOwnRoadmapData = async () => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/timeline/owner?page=1&limit=3`, {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${getToken()}`, // Đính kèm token vào tiêu đề Authorization
                    'Content-Type': 'application/json',
