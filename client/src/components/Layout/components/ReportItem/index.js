...
function ReportItem({ children }) {
    ...
    const fetchConfirmReport = async () => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/report/item/${children.id}`, {
                method: 'PATCH',
                headers: {
    ...
    const fetchBannedRoadmap = async () => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/report/ban/${children.roadmap.id}`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${getToken()}`,
                    'Content-Type': 'application/json',
