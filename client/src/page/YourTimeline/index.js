...
function YourTimeline() {
    ...
    const fetchTimelineData = async (pageNumber) => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/timeline/all?page=${pageNumber}&limit=12`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${getToken()}`,
                    'Content-Type': 'application/json',
    ...
    const fetchGetGroupDivisionByTimeline = async (timelineId) => {
        try {
            const response = await fetch(`http://50.112.48.169:3004/group-division/timelineId/${timelineId}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${getToken()}`,
                    'Content-Type': 'application/json'
