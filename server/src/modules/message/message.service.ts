...
export class MessageService {
    ...
    async create(createMessageDto: CreateMessageDto): Promise<ResponseDto> {
        try {
            const senderResponse = await this.senderService.findOneById(createMessageDto.senderId);
            const sender = Array.isArray(senderResponse.data) ? senderResponse.data[0] : senderResponse.data;
            if (!sender) {
                return {
                    statusCode: 404,
    ...
    async findAllByTeamId(id: number, page: number = 0, limit: number = 10): Promise<ResponseDto> {
        try {
            const messages = await this.messageRepository
                .createQueryBuilder('message')
                .leftJoinAndSelect('message.sender', 'sender')
    ...
    async update(id: number, updateMessageDto: UpdateMessageDto): Promise<ResponseDto> {
        try {
            ...
            if (updateMessageDto.senderId) {
                ...
                sender = Array.isArray(senderResponse.data) ? senderResponse.data[0] : senderResponse.data;
                if (!sender) {
                    return {
                        statusCode: 404,
                        message: 'Sender not found',
            ...
            if (fieldsToUpdate.includes('teamId')) {
                ...
                team = Array.isArray(teamResponse.data) ? teamResponse.data[0] : teamResponse.data;
