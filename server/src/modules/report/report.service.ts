...
export class ReportService {
    ...
    async create(createReportDto: CreateReportDto): Promise<ResponseDto> {
        try {
            ...
            const poster = Array.isArray(posterResponse.data) ? posterResponse.data[0] : posterResponse.data;
    ...
    async handleNotificationFromRabbitMQ(data: Report): Promise<void> {
        ...
        const receiverData = Array.isArray(receiver.data) ? receiver.data[0] : receiver.data;
        if (receiverData) {
            const responseReport = await this.firebaseService.sendPushNotification(
    ...
    async update(id: number, updateReportDto: UpdateReportDto): Promise<ResponseDto> {
        try {
            const reportResponse = await this.findOne(id);
            const report = Array.isArray(reportResponse.data) ? reportResponse.data[0] : reportResponse.data;
    ...
    async banRoadmap(idRoadmap: number): Promise<{
        statusCode: number;
        message: string;
        data: {
            idRoadmap: number;
        ...
        try {
            ...
            if (numberOfReport >= 3) {
                ...
                const deletedRoadmap = Array.isArray(result.data) ? result.data[0] : result.data;
                return {
                    statusCode: 200,
    ...
    async checkReport(id: number): Promise<ResponseDto> {
        try {
            const reportResponse = await this.findOne(id);
            const report = Array.isArray(reportResponse.data) ? reportResponse.data[0] : reportResponse.data;
