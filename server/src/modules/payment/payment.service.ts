...
export class PaymentService {
    ...
    async createByZalopay(createPaymentDto: CreatePaymentDto): Promise<{
        statusCode: number;
        message: string;
        data: Payment;
    }> {
    ...
    async update(
        ...
        try {
            ...
            if (oldPaymentResponse.data.type === 'banking' && typeof file !== 'undefined') {
                ...
                const uploadResponse = await this.cloudinaryService.uploadImage(file);
                updatePaymentDto.image =
                    uploadResponse.secure_url.toString() + ' ' + uploadResponse.public_id.toString();
            }
            const newPayment = this.paymentRepository.create({
    ...
    async getBillByUserId(idUser: number, page: number = 1, limit: number = 10) {
        try {
            const payments = await this.paymentRepository
                .createQueryBuilder('payment')
                .leftJoinAndSelect('payment.user', 'user')
