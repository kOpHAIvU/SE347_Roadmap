...
export class TimelineService {
    ...
    async create(createTimelineDto: CreateTimelineDto): Promise<ResponseDto> {
        try {
            ...
            console.log('Leader: ', createTimelineDto);
            if (leaderResponse.statusCode !== 200) {
                return {
                    statusCode: 404,
            ...
            const leader = Array.isArray(leaderResponse.data) ? leaderResponse.data : leaderResponse.data[0];
    ...
    async findTimelinesByUserId(userId: number, page: number = 1, limit: number = 10): Promise<ResponseDto> {
        try {
            const timelines = await this.timelineRepository
                .createQueryBuilder('timeline')
                .leftJoinAndSelect('timeline.roadmap', 'roadmap')
