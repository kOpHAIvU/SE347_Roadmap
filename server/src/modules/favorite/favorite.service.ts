...
export class FavoriteService {
    ...
    async create(createFavoriteDto: CreateFavoriteDto): Promise<ResponseDto> {
        try {
    ...
    async update(id: number, updateFavoriteDto: UpdateFavoriteDto): Promise<ResponseDto> {
        try {
            ...
            const favorite = Array.isArray(favoriteResponse.data) ? favoriteResponse.data[0] : favoriteResponse.data;
    ...
    async CheckOutUserFavoriteRoadmap(idUser: number, idRoadmap: number): Promise<ResponseDto> {
        try {
            const favorite = await this.favoriteRepository
                .createQueryBuilder('favorite')
